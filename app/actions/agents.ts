...
export async function createAgent(input: CreateAgentInput): Promise<{ id: string; apiKey: string }> {
  ...
  await db.insert(agents).values({ id, workspaceId: workspace.id, name, apiKey: `retired:${id}`, apiKeyHash: hashApiKey(apiKey), keyPrefix: keyPrefix(apiKey), keyCreatedAt: new Date(), os: input.os?.trim() ?? '', status: 'offline' })
