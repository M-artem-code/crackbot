...
export async function saveAiProvider(formData: FormData) {
  ...
  try {
    const provider = createOpenAICompatible({ name: 'preflight', baseURL: baseUrl, apiKey })
...
export async function activateAiProvider(id: string) {
  const [owned] = await db.select({ id: aiProviders.id }).from(aiProviders).where(eq(aiProviders.id, id)).limit(1)
  if (!owned) throw new Error('AI-провайдер не найден')
  await db.transaction(async (tx) => {
    await tx.update(aiProviders).set({ isActive: false, updatedAt: new Date() })
    await tx.update(aiProviders).set({ isActive: true, updatedAt: new Date() }).where(eq(aiProviders.id, id))
  })
  revalidatePath('/settings/ai')
  revalidatePath('/assistant')
}
...
export async function testAiProvider(id: string) {
  const [config] = await db.select().from(aiProviders).where(eq(aiProviders.id, id)).limit(1)
  ...
  try {
    ...
    await db.update(aiProviders).set({ lastTestStatus: 'success', lastTestMessage: message, lastTestedAt: new Date(), updatedAt: new Date() }).where(eq(aiProviders.id, id))
    revalidatePath('/settings/ai')
    return { ok: true, message }
  } catch (error) {
    const message = safeProviderError(error)
    await db.update(aiProviders).set({ lastTestStatus: 'failed', lastTestMessage: message, lastTestedAt: new Date(), updatedAt: new Date() }).where(eq(aiProviders.id, id))
    revalidatePath('/settings/ai')
