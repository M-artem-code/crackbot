...
Ты уже знаешь платформу: боты создаются из шаблонов, сценарии исполняются локально через nodriver/CDP (python bot.py), у ботов есть пулы целевых ссылок, версии bot.py, прогоны и логи. AI Studio анализирует и изменяет bot.py через review/apply flow.
Отвечай по-русски, конкретно и практически. Обычно давай 2–4 варианта решения, помечай рекомендуемый, объясняй компромиссы и следующие шаги. Учитывай реальный контекст ниже и предыдущую историю. Не выдумывай статусы, логи, ботов или возможности. Не обещай, что выполнил действие, если у тебя нет инструмента. Никогда не раскрывай ключи и секреты.`
...
export async function POST(request: Request) {
  try {
    ...
    const [{ config, model }, botRows, runRows, templateRows] = await Promise.all([
      getActiveAiProvider(),
      db.select({ name: bots.name, status: bots.status, targetUrl: bots.targetUrl }).from(bots).orderBy(desc(bots.updatedAt)).limit(12),
      db.select({ status: runs.status, error: runs.error, createdAt: runs.createdAt }).from(runs).orderBy(desc(runs.createdAt)).limit(12),
    ...
    await db.insert(assistantMessages).values({ id: `msg_${randomUUID().replaceAll('-', '')}`, role: 'user', content: latestUser.content.slice(0, 8_000) })
    ...
    const result = streamText({
      ...
      onFinish: async ({ text }) => {
        if (text.trim()) await db.insert(assistantMessages).values({ id: `msg_${randomUUID().replaceAll('-', '')}`, role: 'assistant', content: text.slice(0, 20_000) })
      },
    })
