...
export default async function RunsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const filters = { status: value('status'), source: value('source'), botId: value('botId'), query: value('query'), from: value('from'), to: value('to'), page: Number(value('page') ?? 1) }
  ...
  return <AppShell><main className="flex flex-col gap-6 p-6 lg:p-8">
    <header className="flex flex-col gap-2"><p className="font-mono text-xs uppercase tracking-widest text-primary">Operations</p><h1 className="text-balance text-3xl font-semibold tracking-tight">Прогоны</h1><p className="text-sm text-muted-foreground">История локальных запусков ботов и их результаты.</p></header>
    <form className="grid gap-3 rounded-xl bg-card p-4 ring-1 ring-foreground/5 md:grid-cols-4 xl:grid-cols-6">
      ...
      <select name="source" defaultValue={filters.source ?? 'all'} className="h-9 rounded-md border border-input bg-background px-3 text-sm"><option value="all">Все источники</option>{['manual','retry'].map((item) => <option key={item}>{item}</option>)}</select>
