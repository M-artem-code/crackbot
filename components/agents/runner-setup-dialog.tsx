...
export function RunnerSetupDialog({ open, onOpenChange, agentName, pairingToken, expiresAt }: Props) {
  ...
  async function download() {
    ...
    try {
      ...
      if (!result.downloadUrl?.startsWith('https://')) throw new Error('Ссылка на установщик недоступна')
      await navigator.clipboard.writeText(pairingToken).catch(() => undefined)
      setCopied(true)
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl">
        ...
        <ol className="flex flex-col gap-4 text-sm">
          ...
          <li className="flex gap-3"><span className="flex size-7 shrink-0 items-center justify-center rounded-full bg-primary text-primary-foreground">2</span><div className="flex min-w-0 flex-col gap-2"><p className="font-medium">Скачайте setup и сохраните код подключения</p><p className="leading-relaxed text-muted-foreground">Код одноразовый и действует до {expiry || '10 минут'}. При скачивании он автоматически копируется — вставьте код в setup при первом запуске.</p><div className="flex items-center gap-2 rounded-md border bg-muted p-2"><code className="min-w-0 flex-1 truncate font-mono text-xs text-foreground">{pairingToken || 'Код недоступен'}</code><Button type="button" variant="outline" size="icon-sm" onClick={copyPairingToken} disabled={!pairingToken} aria-label="Скопировать код подключения">{copied ? <CheckIcon /> : <CopyIcon />}</Button></div><div className="flex flex-wrap gap-2"><Button size="sm" className="w-fit" onClick={download} disabled={!pairingToken || downloading}>{downloading ? <Loader2Icon className="animate-spin" data-icon="inline-start" /> : <DownloadIcon data-icon="inline-start" />}Скачать Windows beta</Button><Button type="button" variant="outline" size="sm" onClick={copyPairingToken} disabled={!pairingToken}>{copied ? <CheckIcon data-icon="inline-start" /> : <CopyIcon data-icon="inline-start" />}{copied ? 'Код скопирован' : 'Скопировать код'}</Button></div>{error ? <p className="text-xs text-destructive">{error}</p> : null}</div></li>
